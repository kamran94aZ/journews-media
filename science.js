...
async function fetchData() {
    try {
        ...
        db.articles = (jsonResponse.status === 'success') ? jsonResponse.data.filter(a => a.category === 'science') : [];
        renderAll();
...
function renderAll() {
    ...
    if (!db.articles || db.articles.length === 0) {
        container.innerHTML = `<p style="color: #888;">No secure science records found in the network.</p>`;
        return;
    }
    ...
    db.articles.forEach(article => {
        ...
        articleCard.innerHTML = `
            ...
            <h2 class="article-title" style="margin: 10px 0; color: #fff; font-size: 1.75rem; font-weight: 700;">${article.title}</h2>
            <div class="article-content" style="color: #ccc; font-size: 1.05rem; line-height: 1.6; margin-bottom: 15px; text-align: justify;">
                ${article.content}
            </div>
