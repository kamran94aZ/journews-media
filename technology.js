...
async function fetchData() {
    try {
        // Yalnız technology kateqoriyasını gətirir
        const response = await fetch('http://51.21.245.87:3000/api/articles?category=technology');
        if (!response.ok) {
...
async function addArticle() {
    ...
    const category = "technology"; // Kateqoriya sabitləndi
...
function renderAll() {
    ...
    if (!db.technology || db.technology.length === 0) {
